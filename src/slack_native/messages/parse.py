...
@lru_cache()
def _path_to_emoji(emoji_name: str):
...
def fetch_emojis(emoji_names: tuple[str, ...]) -> dict[str, str | Any] | None:
    ...
    for emoji_name in emoji_names:
        emoji_path = _path_to_emoji(emoji_name)
        if emoji_path is None:
            emoji_paths_to_fetch.append(emoji_name)
    if len(emoji_paths_to_fetch) == 0:
        return {emoji_name: _path_to_emoji(emoji_name) for emoji_name in emoji_names}
    # TODO: dynamically create the URL with the ID of the workspace
    url = furl("https://edgeapi.slack.com/cache/T0266FRGM/emojis/info")
    url.args["fp"] = "af"
    url.args["_x_num_retries"] = 0
    ...
    data = {
        ...
        # Here's where we specify the emojis we want to fetch
