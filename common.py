...
def on_channel_selected(item: QListWidgetItem, messages_manager: MessagesManager, messages_widgets: dict):
    ...
    channel_messages = fetch_messages(channel["id"])

    # Assuming messages_manager has a method to update the UI with new messages
    messages_manager.messages_updated.emit(channel, channel_messages)
...
def update_messages_ui(self, channel):
    ...
    show_channel(channel, self, self.messages_frame[1])
...
class MessagesManager(QObject):
    ...
    def create_messages_page(self, channels: List[dict] | None = None, ):
        ...
        channels_list_widget.itemPressed.connect(
            lambda item: on_channel_selected(item, self, channel_messages_widgets))
        ...
        self.messages_updated.connect(update_messages_ui)
